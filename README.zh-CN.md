...
## 通过 GitHub Actions 部署到飞书

如果你希望电脑关机后仍然每天自动推送，可以把这套能力部署到 GitHub Actions。

这个仓库现在已经包含：

- `scripts/build-bilingual-digest.js`：生成双语摘要
- `scripts/send-feishu-api.js`：直接调用飞书 OpenAPI 发送消息
- `.github/workflows/ai-builder-feishu-digest.yml`：每日定时任务
