...
## GitHub Actions Deployment For Feishu

If you want the digest to keep sending even when your local machine is off, deploy it through GitHub Actions.

What this repository now includes:

- `scripts/build-bilingual-digest.js` for bilingual digest generation
- `scripts/send-feishu-api.js` for direct Feishu OpenAPI delivery
- `.github/workflows/ai-builder-feishu-digest.yml` for daily scheduled runs
